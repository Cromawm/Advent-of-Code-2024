...
# with open("lists.txt") as f: print(f"Day two part two: {len(list(filter(lambda l: ((all(x > y for (x, y) in list(zip(l, l[1:]))) or all(x < y for (x, y) in list(zip(l, l[1:])))) and all(4 > abs(x-y) for (x, y) in list(zip(l, l[1:])))) or any(list(map(lambda l: (all(x > y for (x, y) in list(zip(l, l[1:]))) or all(x < y for (x, y) in list(zip(l, l[1:])))) and all(4 > abs(x-y) for (x, y) in list(zip(l, l[1:]))), [l[:i] + l[i+1:] for i in range(len(l))]))), list(map(lambda record: list(map(int, record.split(" "))), f.readlines())))))}")